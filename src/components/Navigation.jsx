...
const Navigation = () => {
    ...
    return (
        <nav className='bg-[#232528] h-12 flex items-center pl-2 overflow-hidden z-50 sticky top-0'>
            <GiHamburgerMenu className='text-white text-4xl peer md:hidden cursor-pointer md:cursor-none z-20' onClick={() => {
                setOpenMenu(!openMenu)
            }} />

            {(
                <ul className={`${openMenu ? 'translate-x-0' : '-translate-x-40'} z-10 transition-transform bg-[#232528] text-white absolute top-0 left-0 h-screen w-3/12 flex flex-col items-center pt-20 gap-3 md:flex md:flex-row md:w-screen md:h-12 md:p-0 md:m-0 md:z-0 md:translate-x-0 md:items-center
                    `}>
                    ...
                    <Link to='/jobs'><button className='_button-style' >Jobs</button></Link>
